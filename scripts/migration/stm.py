...
__stm_directory = "data/downloads/stm"

async def download_stm():
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(url) as response:
            if response.status == 200:
                await asyncio.to_thread(os.makedirs, __stm_directory, exist_ok=True)
                
                zip_file = f"{__stm_directory}/data.zip"
                ...
                with zipfile.ZipFile(zip_file, "r") as zip:
                    zip.extractall(f"{__stm_directory}")
...
async def init_database_stm(db_name: str, db_username: str, db_passwd: str) -> None:
    ...
    try:
        ...
        answer = input("Do you want to clean up the __stm_directory from txt files? (y/n) ")
        if answer == "yes" or answer == "y":
            print("Cleaning up")
            dir_content = os.listdir(__stm_directory)
            for content in dir_content:
                if os.path.isfile(content) and content.endswith(".txt"):
                    os.remove(f"{__stm_directory}/*.txt")
            print("Cleaned up")
        else:
...
async def __calendar_table(conn: asyncpg.Connection):
    ...
    with open(f"{__stm_directory}/calendar.txt", "r", encoding="utf-8") as file:
        file.readline()
...
async def __calendar_dates_table(conn: asyncpg.Connection):
    ...
    with open(f"{__stm_directory}/calendar_dates.txt", "rb") as file:
...
async def __route_table(conn: asyncpg.Connection):
    ...
    with open(f"{__stm_directory}/routes.txt", "r", encoding="utf-8") as file:
        file.readline()
        records = []
        async with conn.transaction():
            for line in file:
...
async def __shapes_table(conn: asyncpg.Connection):
    ...
    with open(f"{__stm_directory}/shapes.txt", "rb") as file:
        await conn.copy_to_table(
            table_name="Shapes",
            source=file,
            format="csv",
...
async def __stop_times_table(conn: asyncpg.Connection):
    ...
    with open(f"{__stm_directory}/stop_times.txt", "rb") as file:
        await conn.copy_to_table(
...
async def __stops_table(conn: asyncpg.Connection):
    ...
    with open(f"{__stm_directory}/stops.txt", "rb") as file:
        await conn.copy_to_table(
...
async def __trips_table(conn: asyncpg.Connection):
    ...
    with open(f"{__stm_directory}/trips.txt", "rb") as file:
        await conn.copy_to_table(
